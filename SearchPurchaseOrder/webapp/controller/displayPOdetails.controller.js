sap.ui.define([
	...
	return Controller.extend("com.inc.SearchPurchaseOrder.controller.displayPOdetails", {
		onInit: function () {
			...
			jQuery
				.ajax({
					...
					success: function (result) {
						...
						that.viewMatDetails = sap.ui.xmlfragment("viewMatDetails", "com.inc.SearchPurchaseOrder.Fragments.viewMatDetails", that);
		...
		onPressConfirmOrder: function (oEvent) {
			...
			if (!this._pDialog) {
				this._pDialog = Fragment.load({
					id: oView.getId(),
					name: "com.inc.SearchPurchaseOrder.Fragments.POConfirmationFrag",
					controller: this
		...
		onPressConfirmPartialOrder: function (oEvent) {
			...
			if (!this._pDialog) {
				this._pDialog = Fragment.load({
					id: oView.getId(),
					name: "com.inc.SearchPurchaseOrder.Fragments.POPartialConfirmationFrag",
					controller: this
				}).then(function (oDialog) {
					oDialog.attachAfterOpen(this.onDialogAfterOpen, this);
					oView.addDependent(oDialog);
		...
		onDownloadPdf: function () {
			...
			var obj = {
				...
				"invoiceDate": mHeaderDetailsData.POdate,
				"dueDate": null,
				"createdAt": createdAt,
				...
				"discount": null,
				"invoiceTotal": mHeaderDetailsData.NetPrice,
				"sapInvoiceNumber": 0,
				"fiscalYear": null,
				"currency": "EUR",
