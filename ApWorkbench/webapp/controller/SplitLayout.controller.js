sap.ui.define([
...
], function (BaseController, JSONModel, ODataModel, MessageToast, MessageBox, Formatter, formatter, Filter,
	Operator, History, InfoLabel, BusyIndicator) {
	"use strict";
	return BaseController.extend("com.inc.ApWorkbench.controller.SplitLayout", {
		...
		onInit: function () {
			...
			sPDFPath = jQuery.sap.getModulePath("com.inc.ApWorkbench.css.pdfs", "/Test.pdf");
			...
			oPropertyModel = new sap.ui.model.json.JSONModel(jQuery.sap.getModulePath("com.inc.ApWorkbench.model", "/propertyModel.json"));
			...
			// var oInvoiceModel = new sap.ui.model.json.JSONModel(jQuery.sap.getModulePath("com.inc.ApWorkbench.model", "/invItemModel.json"));
			...
			this.oRouter.attachRoutePatternMatched(function (oEvent) {
				...
				if (oEvent.getParameters().arguments.value && oEvent.getParameters().arguments.value != "abc") {
					...
					var url = "InctureApDest/invoiceHeader?requestId=" + value;
					jQuery
						.ajax({
							url: url,
							type: "GET",
							...
							success: function (result) {
								...
								for (var i = 0; i < oInvoiceModel.getData().oInvoiceHeaderDetails.invoiceItems.length; i++) {
									var items = oInvoiceModel.getData().oInvoiceHeaderDetails.invoiceItems;
									items[i].oldInvQty = items[i].invQty;
									items[i].oldItemText = items[i].itemText;
									items[i].oldPrice = items[i].price;
									items[i].oldNetWorth = items[i].netWorth;
		...
		onPressAddPO: function () {
			this.oRouter.navTo("createPO");
		},
		...
		fnVendorSelected: function (oEvent) {
			...
			oInvoiceModel.setProperty("/oInvoiceHeaderDetails/vendorId", com.inc.ApWorkbench.util.Formatter.removeZero(oSelectedItem.Vendor));
		...
		handleGroupDialogConfirm: function () {
			var oTable = this.byId("container-ApWorkbench---invoiceTask--grnSummaryTableId-container-ApWorkbench---invoiceTask--grnTab-0"),
		...
		fnRowActionAccAss: function () {
			if (!this.oAccAssignment) {
				this.oAccAssignment = sap.ui.xmlfragment("com.inc.ApWorkbench.view.fragments.accAssignment", this);
		...
		onOpenMessage: function () {
			if (!this.InvoiceMessage) {
				this.InvoiceMessage = sap.ui.xmlfragment("com.inc.ApWorkbench.view.fragments.message", this);
		...
		onOpenAttachment: function () {
			if (!this.InvoiceAttachment) {
				this.InvoiceAttachment = sap.ui.xmlfragment("com.inc.ApWorkbench.view.fragments.Attachment", this);
		...
		onOpenComment: function () {
			if (!this.commentsFrag) {
				this.commentsFrag = sap.ui.xmlfragment("com.inc.ApWorkbench.view.fragments.comments", this);
		...
		fnOpenLineItemMessages: function (oEvent) {
			...
			if (!this._oLineItemMessagePopover) {
				this._oLineItemMessagePopover = sap.ui.xmlfragment("com.inc.ApWorkbench.view.fragments.lineItemMessages", this);
		...
		//-------------------------*******OPEN ADD PO FRAGMENT************------------------------//
		fnAddPoOpenFrag: function () {
			if (!this.addpo) {
				this.addpo = sap.ui.xmlfragment("com.inc.ApWorkbench.view.fragments.addpo", this);
				this.getView().addDependent(this.addpo);
			}
			...
			if (oEvent.getSource().getSelected()) {
				for (var i = 0; i < modelPOData.length; i++) {
					if (modelPOData[i].deleted == "true" || modelPOData[i].checkBoxSelection == true || modelPOData[i].checkboxVisible == "true") {
						this.getView().byId("container-ApWorkbench---invoiceTask--idtogglePOLine-" + sId).setSelected(true);
					} else {
						this.getView().byId("container-ApWorkbench---invoiceTask--idtogglePOLine-" + sId).setSelected(false);
						break;
					}
				}
			} else {
						...
						success: function (result) {
							...
							if (result.poHistory.length > 0) {
								...
								for (var i = 0; i < oGRNModel.getData().grnUIDtoList[length1].grnItemTotalUIList.length; i++) {
									oGRNModel.getData().grnUIDtoList[length1].grnItemTotalUIList[i].histType = com.inc.ApWorkbench.util.Formatter.histValue(
		...
		fnSubmit: function (oEvent) {
			...
			var infoLabel = new sap.tnt.InfoLabel({
				text: 'The supplier will be notified of the changes to the invoice that you have made'
			});
			hbox.addItem(label);
			this.createHtmlContent("checkFlag");
			if (this.tableFlag)
				hbox.addItem(infoLabel);
			var dialogBox = new sap.m.Dialog({
				title: 'Confirm',
				type: 'Message',
				content: hbox,
		...
		onSubmit: function (oEvent) {
			...
			if (oEvent.getSource().getText() == "Submit") {
				if (this.submitValidate()) {
					...
					//url = "InctureApTest/email/postInvoiceToSAP"
		...
		_fnOnSubmitCall: function (oEvent) {
			...
			if (oEvent.getSource().getText() == "Save") {
				...
				var objectIsNew = jQuery.extend({}, invoiceData.oInvoiceHeaderDetails);
				delete objectIsNew.invoiceItems;
				if (objectIsNew.lifecycleStatus == "11") {
					objectIsNew.docStatus = "D";
					objectIsNew.manualpaymentBlock = "B";
				}
				for (var i = 0; i < invoiceData.oInvoiceHeaderDetails.invoiceItems.length; i++) {
					// invoiceData.oInvoiceHeaderDetails.invoiceItems[i].changed && 
					if (!invoiceData.oInvoiceHeaderDetails.invoiceItems[i].isDeleted)
				...
				var newObject = {
					"invoiceHeader": objectIsNew,
					"invoiceItems": arrayIsNew,
					"emaiBody": this.createHtmlContent()
				};
		...
		onReject: function () {
			...
			if (!this.rejectFragment) {
				this.rejectFragment = sap.ui.xmlfragment("com.inc.ApWorkbench.view.fragments.rejectDialog", this);
		...
		onDemandPDF: function () {
			this._addPdfContentArea();
			this.byId("invoiceHeaderResponsiveGrid").setDefaultSpan("L6 M6 S12");
		...
		_removeContentArea: function () {
			...
			this.byId("invoiceHeaderResponsiveGrid").setDefaultSpan("L3 M6 S12");
			...
			if (dFlag) {
				...
				if (invLineData) {
					for (var i = 0; i < invLineData.length; i++) {
						var id = "container-ApWorkbench---invoiceTask--toggleBtn-container-ApWorkbench---invoiceTask--idInvoiceTable-" + i;
		...
		onPressManualMatching: function () {
			// var oFragmentName = "com.inc.ApWorkbench.view.fragments.manualMatching";
		...
		fnMatchBtnPress: function (oEvent) {
			...
			var oInvItemSel = sap.ui.getCore().byId("container-ApWorkbench---invoiceTask--idUMInvTbl").getSelectedItem();
			if (oInvItemSel) {
				var objectIsNew = jQuery.extend({}, invoiceData.oInvoiceHeaderDetails);
				delete objectIsNew.invoiceItems;
				...
				$.ajax({
					...
					success: function (result, xhr, data) {
						...
						oInvoiceModel.setProperty("/oInvoiceHeaderDetails", result.invoiceHeader);
						oInvoiceModel.setProperty("/oInvoiceHeaderDetails/invoiceItems", result.invoiceItems);
						that._getMatchedInvItems();
						that._getUnmatchedInvItems();
						that._getUnmatchedPoItems();
						sap.ui.getCore().byId("container-ApWorkbench---invoiceTask--idUMInvTbl").removeSelections();
						sap.ui.getCore().byId("container-ApWorkbench---invoiceTask--idMatInvTbl").removeSelections();
						that.closeBusyDialog();
					}.bind(that),
					error: function (result, xhr, data) {
						//that.closeBusyDialog();
		...
		fnConfirmUM: function (oEvent) {
			//this.openBusyDialog();
			var oInvMatchedTbl = sap.ui.getCore().byId("container-ApWorkbench---invoiceTask--idMatInvTbl");
			var oInvoiceModel = this.getView().getModel("oInvoiceModel"),
				invoiceData = oInvoiceModel.getData();
		...
		fnFilterTableData: function (oEvent) {
			...
			if (oEvent) {
				...
				if (sSearchFieldId == "idMatchedInvoice") {
					sId = "container-ApWorkbench---invoiceTask--idMatInvTbl";
					...
					if (sap.ui.getCore().byId("container-ApWorkbench---invoiceTask--idUnmatchedInvoiceSelect").getSelectedKey() == "all") {
						aMetaModel = ["itemText", "vendorMaterialId", "upcCode"];
					} else {
						aMetaModel = [sap.ui.getCore().byId("container-ApWorkbench---invoiceTask--idUnmatchedInvoiceSelect").getSelectedKey()];
					}
					sId = "container-ApWorkbench---invoiceTask--idUMInvTbl";
				} else {
					if (sap.ui.getCore().byId("container-ApWorkbench---invoiceTask--idUnmatchedPOSelect").getSelectedKey() == "all") {
						aMetaModel = ["shortText", "poVendMat", "poUPC", "documentNumber", "documentItem"];
					} else {
						aMetaModel = [sap.ui.getCore().byId("container-ApWorkbench---invoiceTask--idUnmatchedPOSelect").getSelectedKey()];
					}
					sId = "container-ApWorkbench---invoiceTask--idPoTbl";
				}
		...
		calculateInvoiceAmt: function (oEvt) {
			var index = oEvt.getSource().getBindingContext("oInvoiceModel").getPath().split("/").pop();
			if (Number(oEvt.getParameter("value")) !== 0) {
				...
				oEvt.getSource().setValueState("Error");
				oEvt.getSource().setValue("");
				MessageBox.error("Value can't be Zero(0.00)!");
			}
		},
		...
		onSendMail: function () {
			var oHTMLContent = this.createHtmlContent();
			var that = this;
			var oEMLContent = 'From: ' + "sowndharya.k@incture.com" + '\n';
			oEMLContent = 'To: ' + "selvapriya.d@incture.com" + '\n';
			oEMLContent += 'Subject: ' + "Test implemenation" + "\n";
			oEMLContent += 'X-Unsent: 1' + '\n';
			oEMLContent += 'Content-Type: text/html' + '\n';
			oEMLContent += '' + '\n';
			oEMLContent += oHTMLContent;
			that.blobData = new Blob([oEMLContent], {
				type: 'text/plain'
			});

			var url = window.URL.createObjectURL(that.blobData);
			var a = document.createElement("a");
			...
			var mm = (oDate.getMonth() + 1).toString(); // getMonth() is zero-based
			var dd = oDate.getDate().toString();
			var hh = oDate.getHours().toString();
			var Mm = oDate.getMinutes().toString();
			var ss = oDate.getSeconds().toString();
			var date = yyyy + "-" + (mm[1] ? mm : "0" + mm[0]) + "-" + (dd[1] ? dd : "0" + dd[0]) + "_" + hh + "." + Mm + "." + ss;
			a.download = "Reconciliation_mail " + date + ".eml";
		...
		createHtmlContent: function (checkFlag) {
			var invoiceData = this.getView().getModel("oInvoiceModel").getData().oInvoiceHeaderDetails.invoiceItems;
			var displayArray = [];
			this.tableFlag = false;
			for (var i = 0; i < invoiceData.length; i++) {
				if (invoiceData[i].itemText != invoiceData[i].oldItemText || invoiceData[i].invQty != invoiceData[i].oldInvQty || invoiceData[i].price !=
					invoiceData[i].oldPrice || invoiceData[i].netWorth != invoiceData[i].oldNetWorth) {
					this.tableFlag = true;
					displayArray.push({
						"sNo": i + 1,
						"itemCode": invoiceData[i].itemCode,
						"itemText": invoiceData[i].oldItemText,
						"qty": invoiceData[i].oldInvQty,
						"unitPrice": invoiceData[i].oldPrice,
						"netPrice": invoiceData[i].oldNetWorth,
						"version": "Old"
					});
					displayArray.push({
						"sNo": i + 1,
						"itemCode": invoiceData[i].itemCode,
						"itemText": invoiceData[i].itemText,
						"qty": invoiceData[i].invQty,
						"unitPrice": invoiceData[i].price,
						"netPrice": invoiceData[i].netWorth,
						"version": "New"
					});
				}
			}
			if (!checkFlag) {
				var oHTMLStart = "<html><body>";
				//var oHeader
				if (!this.tableFlag) {
					var oBody = "<h3>" + "Your invoice " + this.getView().getModel("oInvoiceModel").getData().oInvoiceHeaderDetails.extInvNum +
						" sent on " + this.getView().getModel("oInvoiceModel").getData().oInvoiceHeaderDetails.invoiceDate +
						" has been processed Successfully" +
						"</h3>";
				} else {
					var oBody = "<h3>" + "Your invoice " + this.getView().getModel("oInvoiceModel").getData().oInvoiceHeaderDetails.extInvNum +
						" sent on " + this.getView().getModel("oInvoiceModel").getData().oInvoiceHeaderDetails.invoiceDate +
						" has been processed Successfully with the following modifications" + "</h3>";
					oBody +=
						"<table><tr><th style='font-family: Tahoma; font-size: 8pt; wrapping: true; width: 20px; border: solid #000000 1.0pt; border-collapse: collapse;'>S.No</th>";
					oBody +=
						"<th style='font-family: Tahoma; font-size: 8pt; wrapping: true; width: 20px; border: solid #000000 1.0pt; border-collapse: collapse;'>Item Code&nbsp</th>";
					oBody +=
						"<th style='font-family: Tahoma; font-size: 8pt; wrapping: true; width: 20px; border: solid #000000 1.0pt; border-collapse: collapse;'>Item Text&nbsp</th>";
					oBody +=
						"<th style='font-family: Tahoma; font-size: 8pt; border: solid #000000 1.0pt; border-collapse: collapse;'>Quantity&nbsp</th>";
					oBody +=
						"<th style='font-family: Tahoma; font-size: 8pt; border: solid #000000 1.0pt; border-collapse: collapse;'>Unit Price&nbsp</th>";
					oBody +=
						"<th style='font-family: Tahoma; font-size: 8pt; wrapping: true; width: 20px; border: solid #000000 1.0pt; border-collapse: collapse;'>Net price&nbsp</th>";
					oBody +=
						"<th style='font-family: Tahoma; font-size: 8pt; wrapping: true; width: 20px; border: solid #000000 1.0pt; border-collapse: collapse;'>Version&nbsp</th>";
					for (var j = 0; j < displayArray.length; j++) {
						if (!displayArray[j].itemCode)
							displayArray[j].itemCode = "";
						if (!displayArray[j].itemText)
							displayArray[j].itemText = "";
						if (!displayArray[j].qty)
							displayArray[j].qty = "";
						if (!displayArray[j].unitPrice)
							displayArray[j].unitPrice = "";
						if (!displayArray[j].netPrice)
							displayArray[j].netPrice = "";

						oBody += "<tr>";
						oBody += "<td style='font-family: Tahoma; font-size: 8pt; border: solid #000000 1.0pt; border-collapse: collapse;' >" +
							displayArray[j].sNo +
							"</td><td style='font-family: Tahoma; font-size: 8pt; border: solid #000000 1.0pt; border-collapse: collapse;'>" + displayArray[
								j].itemCode +
							"</td><td style='font-family: Tahoma; font-size: 8pt; border: solid #000000 1.0pt; border-collapse: collapse;'>" + displayArray[
								j].itemText +
							"</td><td style='font-family: Tahoma; font-size: 8pt; border: solid #000000 1.0pt; border-collapse: collapse;'>" + displayArray[
							...
							"</td><td style='font-family: Tahoma; font-size: 8pt; border: solid #000000 1.0pt; border-collapse: collapse;'>" + displayArray[
								j].unitPrice +
							"</td><td style='font-family: Tahoma; font-size: 8pt; padding-left: 1rem; border: solid #000000 1.0pt; border-collapse: collapse;'>" +
							displayArray[j].netPrice +
							"</td><td style='font-family: Tahoma; font-size: 8pt; padding-left: 1rem; border: solid #000000 1.0pt; border-collapse: collapse;'>" +
							displayArray[j].version +
							"</td></tr>";
					}
					oBody += "</table><br>";
				}
				var oHTMLEnd = "</body></html>";
				var oHTMLContent = oHTMLStart + oBody + oHTMLEnd;
				return oHTMLContent;
			}
		},

		onSubmitPOTask: function () {
			var oInvoiceModel = this.getModel("oInvoiceModel");
			var reqId = oInvoiceModel.getData().oInvoiceHeaderDetails.requestId;
			var url = "InctureApDest/email/updateWorkflowStatus?subjectId=" + reqId;
			var that = this;
			$.ajax({
				url: url,
				method: "GET",
		...
		onValidateInvoice: function (oEvent) {
			if (!this.validateFrag) {
				this.validateFrag = sap.ui.xmlfragment("com.inc.ApWorkbench.view.fragments.validate", this);
				this.getView().addDependent(this.validateFrag);

			}
			var oInvoiceModel = this.getModel("oInvoiceModel");
			this.validateFrag.setModel(oInvoiceModel, "oInvoiceModel");
			this.getremidianStatus();
			this.validateFrag.open();
		},
		getremidianStatus: function (oEvent) {
			var oInvoiceModel = this.getView().getModel("oInvoiceModel");
			var reqId = oInvoiceModel.getData().oInvoiceHeaderDetails.requestId;
			var url = "InctureApDest/invoiceHeader/getRemitanceStatus?requestId=" + reqId;
			var that = this;
			jQuery
				.ajax({
					url: url,
					...
					dataType: "json",
					success: function (result) {
						var status = result.remidianStatus;
						that.getView().getModel("oInvoiceModel");
						if (status === "SAP Posting Success") {
							oInvoiceModel.setProperty("/postBtnvisible", false);
							oInvoiceModel.setProperty("/remidianStatus", status);
							oInvoiceModel.setProperty("/remidianState", "Success");
						} else if (status === "Ready to Post") {
							oInvoiceModel.setProperty("/postBtnvisible", true);
							oInvoiceModel.setProperty("/remidianStatus", status);
							oInvoiceModel.setProperty("/remidianState", "Success");
						} else {
							oInvoiceModel.setProperty("/postBtnvisible", false);
							oInvoiceModel.setProperty("/remidianStatus", status);
							oInvoiceModel.setProperty("/remidianState", "Error");
						}
					}.bind(this)
				});
		},
		onClosevalidatefrag: function () {
			this.validateFrag.close();
		},
		onOpenActivityLog: function (oEvent) {
			if (!this.fragActivityLog) {
				this.fragActivityLog = sap.ui.xmlfragment("com.inc.ApWorkbench.view.fragment.activityLog", this);
				this.getView().addDependent(this.fragActivityLog);
			}
			this.fragActivityLog.open();
		},
		fncloseActivityLog: function () {
			this.fragActivityLog.close();
		},
		onpressVendorBalance: function (oEvent) {
			if (!this.vendorbalances) {
				this.vendorbalances = sap.ui.xmlfragment("com.inc.ApWorkbench.view.fragment.vendorbalances", this);
				this.getView().addDependent(this.vendorbalances);
			}
			this.vendorbalances.open();
		},
		fncloseVendorBlc: function () {
			this.vendorbalances.close();
		},
		navToTaskInbox: function () {
			location.href = "https://cw4quuwhzv0yfdhktaskapp-approuter.cfapps.eu10.hana.ondemand.com/comincZTaskApp-1.0.0/index.html";
		}
